...
boolean verify(byte* payload)
{
  ...
  String data = str_payload.substring(44);

  if (now_ts - ts_long > timeout) {
      return false;
  }

  String hash_data = str_payload.substring(33) + '|' + secret_key;
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (verify(payload)) {
    String str_payload = String((char*)payload);
    String state = str_payload.substring(44);
    Serial.println(topicStr + " " + state);
    
    if (topicStr == "smarthome/living-room/light/device1") // Led 8 on board 8 relay 
    {
       if(state == "1") {
