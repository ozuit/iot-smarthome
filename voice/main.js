...
app.post('/iotagent', async (req, res) => {
  ...
  if (transcription[0]) {
    ...
    } else if (responseText.indexOf('chế độ') != -1 && responseText.indexOf('đi làm') != -1) {
      axiosInstance.put(`/api/${process.env.INTERNAL_TOKEN}/iot-agent/turn-off-all`)
    } else if (responseText.indexOf('chế độ') != -1 && responseText.indexOf('đi ngủ') != -1) {
      axiosInstance.put(`/api/${process.env.INTERNAL_TOKEN}/iot-agent/sleep-mode`)
    } else if (responseText.indexOf('chế độ') != -1 && responseText.indexOf('xem phim') != -1) {
      axiosInstance.put(`/api/${process.env.INTERNAL_TOKEN}/iot-agent/movie-mode`)
    } else if (responseText.indexOf('chế độ') != -1 && responseText.indexOf('đọc sách') != -1) {
